...
class MLIntentDetector(BaseIntentDetector):
    ...
    def __init__(
        ...
        if self._model is None:
            self._model = self._safe_load_model(self._resolve_model_path(model_path))

    def _resolve_model_path(self, model_path: str) -> str:
        """Resolve model path: try as-is, then relative to package root."""
        p = Path(model_path)
        if p.exists():
            return str(p)
        # Try relative to the package root (src/ignis_router/)
        pkg_root = Path(__file__).resolve().parents[1]
        bundled = pkg_root / model_path
        if bundled.exists():
            return str(bundled)
        # Try with subdir structure (models/knnrouter/knnrouter.pkl)
        filename = p.name
        stem = p.stem
        bundled_subdir = pkg_root / "models" / stem / filename
        if bundled_subdir.exists():
            return str(bundled_subdir)
        return model_path
...
class SemanticIntentDetector(BaseIntentDetector):
    """
    Intent detection using Sentence Transformer + Logistic Regression.

    Uses semantic embeddings for accurate intent classification.
    ...
    def _load_classifier(self, model_path: Optional[str]) -> None:
        try:
            ...
            self._classifier = SemanticIntentClassifier.load(path=model_path)
            logger.info("Semantic intent classifier loaded successfully.")
        except Exception as exc:
            logger.warning("Failed to load semantic intent classifier: %s", exc)
            self._classifier = None

    @property
    def is_available(self) -> bool:
        return self._classifier is not None and self._classifier.is_available
    ...
    def detect_intent(self, text: str) -> tuple[Intent, float]:
        ...
        try:
            intent_str, confidence, _ = self._classifier.predict(text)
            intent = _coerce_intent(intent_str)
            return intent, round(confidence, 3)
        except Exception as exc:
            logger.warning("Semantic intent prediction failed: %s", exc)
            return Intent.GENERAL_CHAT, 0.0

    def get_model_hint(self) -> Optional[str]:
        """Semantic classifier predicts intents, not model names."""
        return None

    def clear_model_hint(self) -> None:
        """No-op for semantic classifier."""
        pass
...
class HybridIntentDetector(BaseIntentDetector):
    ...
    def __init__(
        self,
        ml_detector: "BaseIntentDetector",
